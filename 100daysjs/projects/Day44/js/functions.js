...
async function getRepos(username){
    ...
    if(!response.ok){
        errorMessage("User repos not found, try another");
    }
    else {
...
export function errorMessage(message) {
    ...
    return v.repos.innerHTML = `<p class="alert alert-danger">${message}</p>`;
}

//Displays the user data
export function displayData(user) {
    const html = `
    <img src="${user.avatar_url}" alt="${user.login}" class="img-thumbnail rounded-circle">
    <h2>${user.name}</h2>
    <p>${user.login}</p>
    <div class="d-grid">
